<#
.SYNOPSIS
Retrieves and sorts event data from WDAC logs, synthesizes data, and generates sample rules in a WDAC policy XML.
Alternatively, the procedure can export the event data to a tab-delimited CSV output, PSCustomObjects, or as an Excel worksheet.

.DESCRIPTION
Get-WDACEvents.ps1 retrieves WDAC event data from live or saved event logs on the local or a remote computer in a manner that makes analysis much easier than the raw data itself.
In addition to reporting the raw data from the logs, Get-WDACEvents.ps1 synthesizes data so that commonalities between events involving different users or computers can be aggregated.
Output can be a WDAC policy XML file (the default), a tab-delimited CSV, an array of PSCustomObjects, or a formatted Excel worksheet.

By default, the script retrieves error and warning events from the "CodeIntegrity - Operational" and "AppLocker - MSI and Script" event logs on the local computer. 
Or you can specify a remote computer from which to gather the events.
For forwarded events, you can retrieve from the ForwardedEvents log, and/or named event logs if you've forwarded WDAC events to log(s) other than to ForwardedEvents.
Instead of live logs, you can specify the paths to one or more exported .evtx event log files.

WDAC audit mode policies produce warning events ("would have been blocked"), while enforced mode produces error events ("was blocked"). 
You can choose to report only errors, only warnings, or all events (the default).

The -FromDateTime and -ToDateTime options enable you to limit events to time ranges.

Data from each event is correlated with its turned into a line of tab-delimited CSV. Lines are sorted before being output.
...
Use the -EventLevel switch to indicate whether to retrieve errors-only (enforced), warnings-only (audit), or both (default).
Use the -NoMsiAndScript switch to exclude events from the "AppLocker - MSI and Script" log.
Use the -ForwardedEvents switch to read from the ForwardedEvents log instead of from the default WDAC logs.
Use -EventLogNames to specify the names of logs where WDAC events were forwarded if other than the ForwardedEvents log.
